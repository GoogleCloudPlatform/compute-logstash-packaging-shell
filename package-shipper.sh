...
fpm -n logstash-shipper -v $VERSION -a all -C $CURRENT_DIR -m "<lesspublic@gmail.com>" --conflicts logstash-server --pre-install $CURRENT_DIR/logstash.preinstall --post-install $CURRENT_DIR/logstash.postinstall  --description "Logstash Open Source Log Management - Shipper" --url 'http://www.logstash.net/' -t deb --config-files etc/logstash/syslog-shipper.conf --config-files etc/default/logstash-shipper -s dir etc/default/logstash-shipper etc/init.d etc/logstash/syslog-shipper.conf usr/share/logstash/logstash.jar var
